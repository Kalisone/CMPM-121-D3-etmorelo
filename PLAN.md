...
- [] Create a global Map object to function as the "Memento" for storing the state of modified cells (key: cell ID/coordinates, value: cell content).

- [] Refactor grid rendering to separate "intrinsic" state (deterministic generation) from "extrinsic" state (user modifications).
...
- [] Update interaction logic: When a player collects or moves a token, save the new state of that specific cell into the global Map.
